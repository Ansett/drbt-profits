...
const compute = () => {
  ...
  const filteredCalls = props.calls.filter(
    (call) => !call.rug && call.xs >= 10
  );
  const totalX100 = filteredCalls.filter((call) => call.xs >= 100).length;
  const totalX50 = filteredCalls.filter(
    (call) => call.xs >= 50 && call.xs < 100
  ).length;
  const totalX10 = filteredCalls.filter(
    (call) => call.xs >= 10 && call.xs < 50
  ).length;

  for (const call of filteredCalls) {
    const xindex = call.xs >= 100 ? "x100" : call.xs >= 50 ? "x50" : "x10";
    const total =
      call.xs >= 100 ? totalX100 : call.xs >= 50 ? totalX50 : totalX10;
    ...
    category.pct = roundDec((category.count / total) * 100, 1);
  }
