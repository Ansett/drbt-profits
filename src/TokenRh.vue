...
const STATE_STORAGE_KEY = 'state-rh-token-a'
const QUERY_STORAGE_KEY = 'query-rh'
function storeForm() {
...
onMounted(async () => {
  ...
  if (state.rememberQuery) {
    query.value = localStorage.getItem(QUERY_STORAGE_KEY) || ''
  }
...
const createWorker = async () => {
  const WorkerConstructor = (await import('@/worker-rh-token?worker')).default
...
const archives = ref<RhTokenHistory[]>([])
const current = ref<RhTokenHistory | null>(null)
...
async function storeData(rows: (string | number | Date)[][], fileName: string) {
  ...
  if (caIndex < 0) {
    errorFile.value = `ca header not found`
    return
  }
  const ca = String(rows[0][caIndex] ?? '').toLowerCase()
  if (!ca) {
    errorFile.value = `ca header not found`
    return
  }
  const name =
    (rows[0][headers.indexOf('name')] as string) ||
    (rows[0][headers.indexOf('symbol')] as string) ||
    ''

  const snapshots: RhTokenHistory['snapshots'] = []
  ...
  const newArchive: RhTokenHistory = {
    snapshots,
    fileName,
    name,
    ca,
    allFields: headers,
    created: createdFromRow(headers, rows[0]),
  }
...
function createdFromRow(headers: string[], row: (string | number | Date)[]): [string, string] {
  ...
  if (snapshot instanceof Date || (typeof snapshot === 'string' && snapshot))
    return formatDate(snapshot)
...
watch(query, val => {
  if (state.rememberQuery) localStorage.setItem(QUERY_STORAGE_KEY, val || '')
